#!/usr/bin/env python3
"""
Concurrency and performance test suite for the OmniVoice Neuron streaming server.

Tests:
  1. Single request baseline (TTFA, latency, RTF)
...
def run_single_test(server_url, text_key="medium"):
    ...
    for i in range(3):
        ...
        print(
            f"  Run {i+1}: TTFA={m.ttfa:.3f}s  Lat={m.total_latency:.3f}s  "
            f"Audio={m.audio_duration:.2f}s  RTF={m.rtf:.1f}x  [{ok}]"
        )

    ok = [r for r in results if not r.error]
    if ok:
        print(
            f"\n  Avg: TTFA={statistics.mean(r.ttfa for r in ok):.3f}s  "
            f"Lat={statistics.mean(r.total_latency for r in ok):.3f}s  "
            f"RTF={statistics.mean(r.rtf for r in ok):.1f}x"
        )
...
def run_concurrent_test(server_url, concurrency, num_requests=None,
    ...
    with ThreadPoolExecutor(max_workers=concurrency) as pool:
        ...
        for i, f in enumerate(futures):
            m = f.result()
            all_m.append(m)
            ok = "OK" if not m.error else "FAIL"
            print(
...
def print_report(all_results):
    ...
    hdr = f"  {'Test':<30} {'Conc':>4} {'OK':>4} {'Thru':>7} {'P50':>7} {'RTF50':>7}"
    print(f"\n{hdr}")
    print("  " + "-" * (len(hdr) - 2))
    ...
    for r in all_results:
        ...
        print(
            ...
            f"{s['throughput_rps']:6.2f}r {p50:6.3f}s {rtf50:5.1f}x"
        )
...
def main():
    parser = argparse.ArgumentParser(description="OmniVoice Concurrency Test")
    ...
    if not test_health(args.server):
        print("\n  Server not ready. Start with: python src/server.py")
        sys.exit(1)
