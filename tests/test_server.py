#!/usr/bin/env python3
"""
Test suite for the OmniVoice Neuron streaming server.
...
def test_service_info(server_url):
    ...
    try:
        ...
        assert "OmniVoice" in data.get("service", ""), "Missing service name"
...
def test_metrics(server_url):
    ...
    try:
        ...
        return TestResult(
            ...
            passed=True,
            duration=time.perf_counter() - t0,
            details=f"Avg RTF: {data.get('avg_rtf', 'N/A')}x",
        )
    except Exception as e:
...
def test_streaming_request(server_url, text="Hello, this is a streaming test.",
    ...
    try:
        ...
        return TestResult(
            ...
            passed=True,
            duration=duration,
            details=f"Audio: {audio_dur:.2f}s, TTFA: {ttfa:.3f}s, "
                    f"Chunks: {chunks}, RTF: {audio_dur/duration:.1f}x, "
                    f"ReqID: {req_id}",
        )
    except Exception as e:
...
def test_language_support(server_url):
...
def test_hindi_language(server_url):
    """Test Hindi language generation."""
    return test_streaming_request(
        server_url,
        text="भारत की जनसंख्या पूरे यूरोप और अफ्रीका महाद्वीप की जनसंख्या से अधिक है।",
        voice="ballad",
        name_suffix="_hindi",
    )


def test_telugu_language(server_url):
    """Test Telugu language generation."""
    return test_streaming_request(
        server_url,
        text="తెలుగు భాష భారతదేశంలో విస్తృతంగా మాట్లాడబడుతుంది.",
        voice="ash",
        name_suffix="_telugu",
    )


def test_tamil_language(server_url):
    """Test Tamil language generation."""
    return test_streaming_request(
        server_url,
        text="தமிழ் மொழி உலகின் பழமையான மொழிகளில் ஒன்றாகும்.",
        voice="sage",
        name_suffix="_tamil",
    )
...
def test_long_form(server_url):
    ...
    long_text = (
        "Welcome to this comprehensive demonstration of the OmniVoice "
        ...
        "remarkably low latency. The system uses discrete masked diffusion "
        "to generate high quality speech from text in a single stage, "
        "supporting over six hundred languages with zero-shot voice cloning."
    )
...
def main():
    parser = argparse.ArgumentParser(description="OmniVoice Server Test Suite")
    ...
    print(f"\n{'='*70}")
    print(f"  OmniVoice Neuron Streaming Server -- Test Suite")
    ...
    # Language tests
    r = test_language_support(server)
    results.append(r)
    status = "PASS" if r.passed else "FAIL"
    print(f"  [{status}] {r.name} ({r.duration:.3f}s) {r.details or r.error or ''}")

    r = test_hindi_language(server)
    results.append(r)
    status = "PASS" if r.passed else "FAIL"
    print(f"  [{status}] {r.name} ({r.duration:.3f}s) {r.details or r.error or ''}")

    r = test_telugu_language(server)
    results.append(r)
    status = "PASS" if r.passed else "FAIL"
    print(f"  [{status}] {r.name} ({r.duration:.3f}s) {r.details or r.error or ''}")

    r = test_tamil_language(server)
    results.append(r)
    status = "PASS" if r.passed else "FAIL"
    print(f"  [{status}] {r.name} ({r.duration:.3f}s) {r.details or r.error or ''}")
